...
def batch_ddim_sampling(
    ...
    **kwargs,
    ...
    if cfg_scale != 1.0:
        ...
        ## process image embedding token (match scripts/evaluation/inference.py: zeros_like(img) canvas)
        if hasattr(model, 'embedder'):
            if uc_image_hw is not None:
                uh, uw = int(uc_image_hw[0]), int(uc_image_hw[1])
                uc_img = torch.zeros(
                    noise_shape[0], 3, uh, uw, device=model.device, dtype=torch.float32
                )
            else:
    ...
    for _ in range(n_samples):
        ...
        ## Latent -> pixels: keep decode in FP32 (autocast off). At 576x1024, VAE decode
        ## under fp16 autocast often underflows to black frames while latents look fine.
