...
def _dynamicrafter_decode_debug(batch_images: torch.Tensor, out_dir: str, stem: str) -> None:
    ...
    print(
        "[DYNAMICRAFTER_DEBUG_DECODE] %s: dtype=%s finite=%s min=%.4f max=%.4f mean=%.4f shape=%s"
        % (
            stem,
            x.dtype,
            finite,
    ...
    if not finite or x.dim() != 5:
        return
    os.makedirs(out_dir, exist_ok=True)
    _, _, t, _, _ = x.shape
    for t_idx, label in ((0, "t0"), (t - 1, "t_last")):
        im = x[0, :, t_idx].clamp(-1.0, 1.0)
        arr = ((im + 1.0) * 0.5 * 255.0).byte().permute(1, 2, 0).contiguous().numpy()
        Image.fromarray(arr).save(os.path.join(out_dir, f"{stem}_{label}.png"))
...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                        unconditional_guidance_scale=1.0, cfg_img=None, fs=None, text_input=False, multiple_cond_cfg=False, loop=False, interp=False, timestep_spacing='uniform', guidance_rescale=0.0, debug_decode_dir=None, **kwargs):
    ...
    batch_variants = []
    for variant_i in range(n_samples):
        ...
        if ddim_sampler is not None:
            ...
            if _env_truthy("DYNAMICRAFTER_DEBUG_LATENTS"):
                z_dbg = samples.detach().float()
                print(
                    "[DYNAMICRAFTER_DEBUG_LATENTS] after DDIM: dtype=%s finite=%s min=%.4f max=%.4f mean=%.4f"
                    % (
                        z_dbg.dtype,
                        bool(torch.isfinite(z_dbg).all().item()),
                        z_dbg.min().item(),
                        z_dbg.max().item(),
                        z_dbg.mean().item(),
                    )
...
def run_inference(args, gpu_num, gpu_no):
    ...
    ## prompt file setting
    assert os.path.exists(args.prompt_dir), f"Error: prompt_dir not found or not a path: {args.prompt_dir!r}"
    ...
    debug_decode_dir = None
    if _env_truthy("DYNAMICRAFTER_DEBUG_DECODE"):
        debug_decode_dir = os.path.join(args.savedir, "debug_decode")
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(enabled=use_amp):
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            batch_samples = image_guided_synthesis(model, prompts, videos, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
                                args.unconditional_guidance_scale, args.cfg_img, args.frame_stride, args.text_input, args.multiple_cond_cfg, args.loop, args.interp, args.timestep_spacing, args.guidance_rescale, debug_decode_dir=debug_decode_dir)
