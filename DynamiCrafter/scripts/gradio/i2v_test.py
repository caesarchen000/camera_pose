...
class Image2Video():
    ...
    def get_image(self, image, prompt, steps=50, cfg_scale=7.5, eta=1.0, fs=3, seed=123):
        ...
        # 576x1024: full forward in fp32 — mixed precision here often yields NaN/zeroed VAE
        # outputs (black video) on some driver/torch builds; 512/256 stay in autocast.
        use_amp = self.resolution[1] < 1024
        # text cond
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=use_amp):
            text_emb = model.get_learned_conditioning([prompt])

            # img cond (writable copy for torch.from_numpy)
            ...
            cond_images = model.embedder(image_tensor_resized.unsqueeze(0)) ## blc; match VAE crop
