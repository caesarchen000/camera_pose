...
class Image2Video():
    ...
    def get_image(self, image, prompt, steps=50, cfg_scale=7.5, eta=1.0, fs=3, seed=123, image2=None):
        ...
        # Full FP32 forward: mixed precision here has produced incoherent / tiled latents on some
        # torch/xFormers stacks (same family of issues as 1024 VAE decode). Slower but stable.
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            text_emb = model.get_learned_conditioning([prompt])

            # img cond (writable C-contiguous copy: torch.from_numpy rejects read-only buffers)
            im0 = np.ascontiguousarray(np.array(image, copy=True, dtype=np.uint8))
            img_tensor = torch.from_numpy(im0).permute(2, 0, 1).float().to(model.device)
            img_tensor = (img_tensor / 255. - 0.5) * 2

            image_tensor_resized = transform(img_tensor)  # 3,h,w in [-1,1]

            if image2 is not None:
                # Match scripts/evaluation/inference.py load_data_prompts(interp=True):
                # stack [img1 x T/2 | img2 x T/2] along time, encode once, then hybrid concat uses
                # only first/last latent slices (same as image_guided_synthesis).
                im1 = np.ascontiguousarray(np.array(image2, copy=True, dtype=np.uint8))
                img_tensor2 = torch.from_numpy(im1).permute(2, 0, 1).float().to(model.device)
                img_tensor2 = (img_tensor2 / 255. - 0.5) * 2
                image_tensor_resized2 = transform(img_tensor2)
                half = frames // 2
                v1 = repeat(image_tensor_resized, "c h w -> c t h w", t=half)
                v2 = repeat(image_tensor_resized2, "c h w -> c t h w", t=half)
                videos_5d = torch.cat([v1, v2], dim=1).unsqueeze(0)  # 1, c, T, h, w
                z_full = get_latent_z(model, videos_5d)
                img_tensor_repeat = torch.zeros_like(z_full)
                img_tensor_repeat[:, :, 0, :, :] = z_full[:, :, 0, :, :]
                img_tensor_repeat[:, :, -1, :, :] = z_full[:, :, -1, :, :]
                cond_images = model.embedder(videos_5d[:, :, 0, :, :])  # same slice as inference.py img
            else:
                ...
                img_tensor_repeat[:, :, -1:, :, :] = z
                cond_images = model.embedder(image_tensor_resized.unsqueeze(0))
