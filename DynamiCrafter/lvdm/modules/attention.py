...
class CrossAttention(nn.Module):
    ...
    def efficient_forward(self, x, context=None, mask=None):
        # xFormers can be importable while its CUDA ops are unavailable for the
        # current torch/cuda build; in that case, fall back to the vanilla path.
        if not XFORMERS_IS_AVAILBLE:
            return CrossAttention.forward(self, x, context=context, mask=mask)
        spatial_self_attn = (context is None)
        k_ip, v_ip, out_ip = None, None, None
