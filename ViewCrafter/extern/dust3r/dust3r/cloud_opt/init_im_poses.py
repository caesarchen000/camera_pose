...
def fast_pnp(pts3d, focal, msk, device, pp=None, niter_PnP=10):
    ...
    for focal in tentative_focals:
        ...
        obj_pts = np.ascontiguousarray(pts3d[msk].reshape(-1, 3).astype(np.float32))
        img_pts = np.ascontiguousarray(pixels[msk].reshape(-1, 2).astype(np.float32))
        if obj_pts.shape[0] < 4 or img_pts.shape[0] < 4:
            continue

        success, R, T, inliers = False, None, None, None
        # Try OpenCV variants in descending feature order.
        for call in (
