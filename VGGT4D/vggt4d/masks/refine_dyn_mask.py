...
class RefineDynMask:
    ...
    def _refine_mask(self, cam_id: int):
        ...
        # Edge case: no dynamic points survive filtering.
        # Fall back to coarse mask for this frame to keep the pipeline running.
        if dyn_pts.shape[0] == 0:
            return rearrange(coarse_mask, "(h w) -> h w", h=h_img, w=w_img)

        n_clusters = min(30, int(dyn_pts.shape[0]))
